...
public:
  void Load(physics::ModelPtr model, sdf::ElementPtr sdf) override 
  {
    ...
    for (int i = 0; i < 3; ++i)
    {
      ...
      if (!links_[i]) 
      {
        RCLCPP_ERROR(ros_node_->get_logger(), "FuerzasLinksPlugin: no encuentro link %s", link_names_[i].c_str());
      }
      else
      {
...
private:
  ...
  rclcpp::Subscription<geometry_msgs::msg::Wrench>::SharedPtr subs_[3];
  ignition::math::Vector3d               last_forces_[3]{{0,0,0},{0,0,0},{0,0,0}};
  ignition::math::Vector3d               last_torques_[3]{{0,0,0},{0,0,0},{0,0,0}};
};
