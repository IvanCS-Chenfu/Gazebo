  ...
  void Load(physics::ModelPtr model, sdf::ElementPtr sdf) override 
  {
    ...
    for (int i = 0; i < 3; ++i)
    {
      ...
      else
      {
        ...
        RCLCPP_INFO(ros_node_->get_logger(), "Publicando velocidad de joint [%s] en [%s] (rate=%.1f Hz)", joint_names_[i].c_str(), topic_names_[i].c_str(), publish_rate_);
      }
    }
    ...
    update_conn_ = event::Events::ConnectWorldUpdateBegin(std::bind(&VelJointsPlugin::OnUpdate, this, std::placeholders::_1));
  }
    ...
    if (enviar)
    {
      for (int i = 0; i < 3; ++i)
      {
        ...
        {   
          ...
          // prismatic          => m
          const double vel = joints_[i]->GetVelocity(0);
